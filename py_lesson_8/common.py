...
# """
# Напишите функцию process_customers(dataset: CustomerDataset), 
# которая принимает объект типа CustomerDataset (или его наследников) 
# и выводит сводную информацию о клиентах с помощью метода display_info().
# """


# def process_customers(dataset: CustomerDataset):
#     print("# Обработка покупателей...")
...
class PremiumCustomerDataset(CustomerDataset):
    def __init__(self, customers_data, source, membership_level = ["Bronze", "Silver", "Gold"]):
        self.membership_level = [m.lower() for m in membership_level]
        super().__init__(customers_data, source)     
        self.customers_data_all = self.customers_data
        self.customers_data = [c for c in self.customers_data if str(c.get("membership", "")).lower() in self.membership_level]


    def filter_by_membership(self, level):
        return [
            customer
            for customer in self.customers_data_all
            if customer.get("membership").lower() == level.lower()
        ]
...
gold_dataset = PremiumCustomerDataset(customers, "API")
